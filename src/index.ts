...
export namespace Lens {
    //#region - Public API
    export const get = <D, R>(data: D, lens: ($: Selector<D>) => SelectorFor<R>): R => {
    ...
    export const mutate = <D, R>(data: D, lens: ($: Mutator<D>) => MutatorFor<R>, value: R | ((prev: R, index: number, context: Lens.Context) => R)): void => {
        const proxy = createProxy({ value: data, isEach: false, path: [], filters: [] });
        const result = lens(proxy as any);
        const { path } = (result as any)[LENS] as State;
    ...
    export const apply = <D, R>(data: D, lens: ($: Applier<D>) => ApplierFor<R>, value: R | ((prev: DeepReadonly<R>, index: number, context: Lens.Context) => R)): D => {
        const proxy = createProxy({ value: data, isEach: false, path: [], filters: [] });
        const result = lens(proxy as any);
        const { path } = (result as any)[LENS] as State;
        const updater = typeof value === "function" ? (value as (prev: R, index: number, context: Lens.Context) => R) : () => value;
    ...
    export type SelectorFor<E> = { readonly [BRAND_EVAL]: E };
    export type MutatorFor<E> = { readonly [BRAND_TARGET]: E; readonly [BRAND_READONLY]?: never };
    export type ApplierFor<E> = { readonly [BRAND_TARGET]: E; readonly [BRAND_READONLY]?: never };

    // All three are now DataLens
    export type Selector<Eval, Chain = Eval> = DataLens<Eval, Eval, Chain>;
    export type Mutator<Target, Chain = Target> = DataLens<Target, Target, Chain>;
    export type Applier<Target, Chain = Target> = DataLens<Target, Target, Chain>;
    ...
    type PathSegment = { type: "property"; key: string } | { type: "index"; index: number } | { type: "accessor"; name: string; args?: string[] };
    ...
    type State = { value: unknown; isEach: boolean; path: Step[]; filters: FilterOp[] };
    ...
    type MapSelectorLensOf<T extends any[]> = { [K in keyof T]: T[K] | SelectorFor<T[K]> };
    ...
    type DataLens<Target, Eval = Target, Chain = Eval> = {
        ...
        (NonNullable<Chain> extends (infer E)[] | readonly (infer E)[]
            ? {
                  (index: number | SelectorFor<number>): DataLens<WrapTarget<Target, Chain, E>, WrapEval<Eval, Chain, E>, E>;
                  at(index: number | SelectorFor<number>): DataLens<WrapTarget<Target, Chain, E>, WrapEval<Eval, Chain, E>, E>;
                  each(): DataLens<E, E[], E>;
                  ...
                  slice(start: number | SelectorFor<number>, end?: number | SelectorFor<number>): DataLens<never, Eval, Chain>;
                  sort<R extends string | number | bigint | TrhSymbols.Comparable | null | undefined>(
                      target: ($: DataLens<never, ElementOf<Chain>>) => SelectorFor<R>,
                      dir: SortDirection,
        ...
        (NonNullable<Chain> extends Set<infer SV>
            ? {
                  has(value: SV | SelectorFor<SV>): DataLens<never, WrapEval<Eval, Chain, boolean>, boolean>;
                  size(): DataLens<never, WrapEval<Eval, Chain, number>, number>;
              }
            : {}) &
        // Map
        (NonNullable<Chain> extends Map<infer MK, infer MV>
            ? {
                  get(key: MK | SelectorFor<MK>): DataLens<KeepTarget<Target, Chain, MV>, WrapEval<Eval, Chain, MV>, MV>;
                  has(key: MK | SelectorFor<MK>): DataLens<never, WrapEval<Eval, Chain, boolean>, boolean>;
    ...
    //#region - Comparison, Conversion, Operators, and Predicates
    ...
    const performEquality = (a: unknown, b: unknown): boolean => {
    ...
    const resolveTypeOf = (value: unknown): string => {
    ...
    const convertToString = (value: unknown): string | null => {
    ...
    type Predicate<O> =
        | [subject: O | SelectorFor<O>, op: NoInfer<OperatorFor<O, 2>>]
        | [subject: O | SelectorFor<O>, op: NoInfer<OperatorFor<O, 3>>, operand: NoInfer<OperandFor<O, OperatorFor<O, 3>>> | SelectorFor<any>]
        | [
              subject: O | SelectorFor<O>,
              op: NoInfer<OperatorFor<O, 4>>,
              operand1: NoInfer<OperandFor<O, OperatorFor<O, 4>>> | SelectorFor<any>,
              operand2: NoInfer<OperandFor<O, OperatorFor<O, 4>>> | SelectorFor<any>,
          ];
    ...
    const evalPredicate = (pred: any): boolean => {
    ...
    const createProxy = (state: State): any => {
        ...
        const nav = (fn: (v: any) => unknown, s: Step): any => createProxy({ value: isEach ? (value as any[]).map(fn) : fn(value), isEach, path: [...path, s], filters: [] });
    ...
    const parseSortArgs = (args: any[]): { dir: string; nullish: string } => {
    ...
    const sortKeyed = <T extends { key: unknown; idx: number }>(keyed: T[], dir: string, nullish: string): void => {
        keyed.sort((a, b) => {
    ...
    const resolveAtIndex = (arr: any[], step: { index: number; filters?: FilterOp[] }): number => {
    ...
    const resolveCallbackPath = (step: { callback: Function }, item: any, steps: Step[], next: number): Step[] => {
        const elProxy = createProxy({ value: item, isEach: false, path: [], filters: [] });
        const subResult = step.callback(elProxy);
        const subPath = (subResult as any)[LENS].path as Step[];
        return [...subPath, ...steps.slice(next)];
    };

    const matchingIndices = (arr: any[], ops: FilterOp[]): number[] => {
    ...
    const seg = {
        prop: (key: string): PathSegment => ({ type: "property", key }),
        idx: (index: number): PathSegment => ({ type: "index", index }),
        acc: (name: string, ...args: string[]): PathSegment => (args.length > 0 ? { type: "accessor", name, args } : { type: "accessor", name }),
        fromPropStep: (key: string | number): PathSegment => (typeof key === "number" ? seg.idx(key) : seg.prop(key)),
    };

    const doApply = (current: any, steps: Step[], idx: number, updater: (prev: any, index: number, ctx: Lens.Context) => any, ctx: Lens.Context): any => {
    ...
    const doMutate = (current: any, steps: Step[], idx: number, updater: (prev: any, index: number, ctx: Lens.Context) => any, ctx: Lens.Context): void => {
        const step = steps[idx];
        const next = idx + 1;
